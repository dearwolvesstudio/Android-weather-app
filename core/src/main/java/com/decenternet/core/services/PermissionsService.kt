...
class PermissionsService(private val _context: Context, private val _localiser: IStringService) :
    IPermissionsService {

    override fun hasLocationAccess(): Boolean {
        return if (Build.VERSION.SDK_INT >= 23) {
            _context.checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED
        } else true
    }
        ...
        when (requestCode) {
            LocationPermission -> {
                if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    callback.onPermissionSuccess(Manifest.permission.ACCESS_COARSE_LOCATION)
                } else {
                    callback.onPermissionDeclined(Manifest.permission.ACCESS_COARSE_LOCATION)
                }
            }
        }
    }
